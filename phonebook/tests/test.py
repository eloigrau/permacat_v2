...
class ContactTestCase(TestCase):
    ...
    def test_login_page_form(self):
        """Test login user"""
        response = self.client.post(reverse('phonebook:phonebook_login_page'),
                                    {'username': self.user.username, 'password': 'password_test'})
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))
...
class ContactViewTestCase(TestCase):
    ...
    def test_call_login_redirect(self):
        response = self.client.get(reverse('phonebook:phonebook_login_page'))
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_view_lists_contacts(self):
        response = self.client.get(reverse('phonebook:phonebook_lists_contacts'))
        self.assertEqual(response.status_code, 200)

    def test_call_view_edit_contact(self):
        response = self.client.get(reverse('phonebook:phonebook_edit', kwargs={'contact_id': self.contact.id}))
        self.assertEqual(response.status_code, 200)

    def test_call_view_post_edit_contact(self):
        response = self.client.post(reverse('phonebook:phonebook_edit', kwargs={'contact_id': self.contact.id}),
                                    {'firstname': self.contact.firstname, 'lastname': self.contact.lastname,
                                     'email': self.contact.email})
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_view_post_new_contact(self):
        response = self.client.post(reverse('phonebook:phonebook_new_contact'), {'firstname': 'Tesst new firstname',
                                                                       'lastname': 'Test new lastname',
                                                                       'email': 'test@newtest.fr'})
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_view_get_new_contact(self):
        response = self.client.get(reverse('phonebook:phonebook_new_contact'))
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_view_call(self):
        response = self.client.get(reverse('phonebook:phonebook_call', kwargs={'num': self.contact.phone}))
        self.assertEqual(response.status_code, 200)

    def test_call_view_exports_contacts(self):
        response = self.client.get(reverse('phonebook:phonebook_exports_contacts'))
        self.assertEqual(response.status_code, 200)

    def test_call_view_delete_contact(self):
        response = self.client.get(reverse('phonebook:phonebook_delete', kwargs={'contact_id': self.contact.id}))
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_logout(self):
        response = self.client.get(reverse('phonebook:phonebook_logout'))
        self.assertRedirects(response, reverse('phonebook:phonebook_login_page'))

    def test_call_view_edit_contact_with_fail(self):
        response = self.client.get(reverse('phonebook:phonebook_edit', kwargs={'contact_id': 9999}))
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))

    def test_call_view_post_search_contact(self):
        response = self.client.post(reverse('phonebook:phonebook_search_contact'), {'query': 'test@newtest.fr'})
        self.assertRedirects(response, reverse('phonebook:phonebook_search_contact_query', kwargs={'query': 'test@newtest.fr'}))

    def test_call_view_get_search_contact(self):
        response = self.client.get(reverse('phonebook:phonebook_search_contact'))
        self.assertRedirects(response, reverse('phonebook:phonebook_lists_contacts'))
