...
class TemplateHelper:
    ...
    def get_theme_variables(scope):
        if scope in settings.THEME_VARIABLES:
            return settings.THEME_VARIABLES[scope]
        else:
            return "ScopeInconnu_"+str(scope)
    ...
    def set_layout(view, context={}):
        ...
        # Get module path
        module = f"dashboard.templates.{settings.THEME_LAYOUT_DIR.replace('/', '.')}.bootstrap.{layout}"
        ...
        if util.find_spec(module) is not None:
            ...
            module = f"dashboard.templates.{settings.THEME_LAYOUT_DIR.replace('/', '.')}.bootstrap.default"
