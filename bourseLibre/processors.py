...
def navbar(request):
    def complicated_query():
        ...
        if request.user.is_authenticated:
            ...
            #        qs_ateliers = qs_ateliers.exclude(asso__slug=nomAsso)
            #        qs_projets = qs_projets.exclude(asso__slug=nomAsso)
             #       qs_projets_prop = qs_projets_prop.exclude(asso__slug=nomAsso)
