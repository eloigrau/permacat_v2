...
from bourseLibre.settings import STATIC_URL
...
SUMMERNOTE_THEME_FILES = {
    ...
    'bs4': {
        ...
        'base_js': (
            'summernote/jquery-3.3.1.min.js',
            'summernote/bootstrap.bundle.min.js',
        ),
        'default_css': (
...
def using_config(_func=None):
    ...
    def decorator(func):
        ...
        def inner_dec(*args, **kwargs):
            ...
            g[var_name] = apps.get_app_config('local_summernote').config
    ...
    try:
        ...
        if not issubclass(klass, AbstractAttachment):
            raise ImproperlyConfigured(
                ...
                "inherited from 'local_summernote.models.AbstractAttachment'" % config["attachment_model"]
            )
        return klass
    except ValueError:
...
def get_attachment_storage():
    ...
    config = apps.get_app_config('local_summernote').config
